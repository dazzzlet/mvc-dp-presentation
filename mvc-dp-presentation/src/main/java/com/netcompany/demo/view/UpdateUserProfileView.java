...
public class UpdateUserProfileView extends AbstractView<UserModel> {
    ...
    public UpdateUserProfileView(Console console) {
    ...
    public EventArg updateModel(UserModel model) {
        if (model.isSuccess() != null) {
            ...
            System.out.println("Update my profile - " + model.getUser().getUsername());
            System.out.println("      ---");
            System.out.println();
        }
    ...
    private EventArg renderForm(User user) {
        ...
        UserUpdateEventArg updateArg = new UserUpdateEventArg(newBio);
        return updateArg;
    }
